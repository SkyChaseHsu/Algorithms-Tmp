def paint(filename, title):
	...
	for data_name in data_names:
		# patch for the color
		if data_name.find('DIV') >= 0:
			color[color_flag] = 'orange'
		else:
			color[color_flag] = 'blue'
...
def genTime(base_time, N, base_N, gen_type):
	...
	if gen_type == 1:
		...
		per_time = base_time / base_N**2
		out_time = per_time * N**2
		return round(out_time,4)

		
# getTheory('expri_data.csv')
# getTheory('expri_data_div.csv')
# getTheory('expri_data_vio.csv')

paint('expri_data.csv', 'The Closest Pair Problem')
paint('expri_data_div.csv', 'The Closest Pair Problem')
paint('expri_data_vio.csv', 'The Closest Pair Problem')
